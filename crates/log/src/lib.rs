...
use crate::error::NihilityLogError;
...
impl Default for LogConfig {
    fn default() -> Self {
        Self {
            log: vec![
                LogConfigChunk::default(),
                LogConfigChunk {
                    enable: false,
                    out_type: LogOutType::File("log".to_string()),
...
pub fn init() -> Result<(), NihilityLogError> {
    let configs =
        nihility_config::get_config::<LogConfig>(env!("CARGO_PKG_NAME"))?.log;
